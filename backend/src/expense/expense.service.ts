...
export class ExpenseService {
  ...
  async getStatistics(req: Request): Promise<any> {
    try {
      ...
      const groupExpenses = (type: string) => expenses
      ...
      const  totalExpenses = Object.values(groupExpenses('expense')).reduce((sum, amount) => sum + amount, 0);
      const totalIncome = Object.values(groupExpenses('income')).reduce((sum, amount) => sum + amount, 0);
